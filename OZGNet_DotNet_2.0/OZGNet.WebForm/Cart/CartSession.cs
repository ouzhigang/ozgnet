...
{
    /// <summary>
    /// Session购物车,实现ICart接口
    /// </summary>
    public class CartSession : ICart
    {
        ...
        public CartSession(string _CartName, string _CartObjectsName)
        {
        ...
        {
            if (!OZGNet.WebForm.Utility.ExistsSession(this._CartName))
            {
                Dictionary<object, int> map = new Dictionary<object, int>();
                HttpContext.Current.Session.Add(this._CartName, map);
                return map;
            }
            else
            {                
                return HttpContext.Current.Session[this._CartName] as Dictionary<object, int>;
            }
        }
        #endregion
        ...
        {
            if (!OZGNet.WebForm.Utility.ExistsSession(this._CartObjectsName))
            {
                Dictionary<object, object> map = new Dictionary<object, object>();
                HttpContext.Current.Session.Add(this._CartObjectsName, map);
                return map;
            }
            else
            {
                return HttpContext.Current.Session[this._CartObjectsName] as Dictionary<object, object>;
            }
        } 
        #endregion
        ...
        public void Add(object pid, int pnum, object obj)
        {
            ...
            if (!has)
            {
                ...
                //保存购物车
                HttpContext.Current.Session[this._CartName] = CartList;

                if (obj != null)
                {
                    CartObjects.Add(pid, obj);
                    HttpContext.Current.Session[this._CartObjectsName] = CartObjects;
                }
            }
            else
            {
