...
{
    /// <summary>
    /// Odbc专用(依赖AspNetPager.dll),实现IDataHelper接口
    /// </summary>
    public class OdbcHelper : IDataHelper
    {
        ...
        public bool ExecuteNonQueryTransaction(IList<string> sqls)
        {
            using (conn = new OdbcConnection())
            {
                ...
                OdbcTransaction st = conn.BeginTransaction();

                int sql_count = sqls.Count;

                //先用泛型动态获取OdbcCommand
                Cmds = new List<OdbcCommand>();
                for (int i = 0; i < sql_count; i++)
                {
                    Cmd = new OdbcCommand(sqls[i], conn);
                    Cmd.Transaction = st;
                    Cmds.Add(Cmd);
                }
        ...
        public IDataReader AspNetPagerProcedure(Wuqi.Webdiyer.AspNetPager pager, string sql)
        {
            ...
            parameters.Add(new OdbcParameter("@pageindex", pager.CurrentPageIndex));
            parameters.Add(new OdbcParameter("@pagesize", pager.PageSize));
            parameters.Add(new OdbcParameter("@docount", "0"));
        ...
        protected void PrepareCommand(CommandType cmd_type, string sql, IList<IDataParameter> parameters)
        {
            ...
            if (parameters != null)
            {
                ...
                {
                    if (sp is OdbcParameter)
                    {
                        Cmd.Parameters.Add(sp);
                    }
                    else
                    {
                        throw new Exception("参数IDataParameter中含有非OdbcParameter的对象");
                    }
                }
            }
        }
        ...
        {
            return new OdbcConnection(this._connstring);
        }
