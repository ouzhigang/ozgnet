...
{
    /// <summary>
    /// MimerSQL专用(依赖AspNetPager.dll),实现IDataHelper接口
    /// </summary>
    public class MimerHelper : IDataHelper
    {
        ...
        /// </summary>
        public MimerCommand Cmd;
        /// <summary>
        /// 获取当前执行的ExecuteNonQueryTransaction()所有Command对象
        /// </summary>
        public IList<MimerCommand> Cmds;
        private MimerDataReader reader;
        private MimerDataAdapter da;
        ...
        public MimerHelper(string connstring)
        {            
        ...
        {
            using (conn = new MimerConnection())
            {
        ...
        public bool ExecuteNonQueryTransaction(IList<string> sqls)
        {
            using (conn = new MimerConnection())
            {
                ...
                MimerTransaction st = conn.BeginTransaction();

                int sql_count = sqls.Count;

                //先用泛型动态获取MimerCommand
                Cmds = new List<MimerCommand>();
                for (int i = 0; i < sql_count; i++)
                {
                    Cmd = new MimerCommand(sqls[i], conn);
                    Cmd.Transaction = st;
                    Cmds.Add(Cmd);
                }
        ...
        public IDataReader AspNetPagerProcedure(Wuqi.Webdiyer.AspNetPager pager, string sql)
        {
            ...
            parameters.Add(new MimerParameter("@pageindex", pager.CurrentPageIndex));
            parameters.Add(new MimerParameter("@pagesize", pager.PageSize));
            parameters.Add(new MimerParameter("@docount", "0"));
        ...
        protected void PrepareCommand(CommandType cmd_type, string sql, IList<IDataParameter> parameters)
        {
            ...
            Cmd = new MimerCommand(sql, conn);
            Cmd.CommandType = cmd_type;
            if (parameters != null)
            {
                foreach (IDataParameter sp in parameters)
                {
                    if (sp is MimerParameter)
                    {
                        Cmd.Parameters.Add(sp);
                    }
                    else
                    {
                        throw new Exception("参数IDataParameter中含有非MimerParameter的对象");
                    }
                }
            }
        }
        ...
        {
            return new MimerConnection(this._connstring);
        }
