...
{
    /// <summary>
    /// Sybase专用(依赖AspNetPager.dll),实现IDataHelper接口
    /// </summary>
    public class SybaseHelper : IDataHelper
    {
        private string _connstring;
        private SybaseConnection conn;
        /// <summary>
        /// 获取当前执行的Command(ExecuteNonQueryTransaction()除外)
        /// </summary>
        public SybaseCommand Cmd;
        /// <summary>
        /// 获取当前执行的ExecuteNonQueryTransaction()所有Command对象
        /// </summary>
        public IList<SybaseCommand> Cmds;
        private SybaseDataReader reader;
        private SybaseDataAdapter da;
        ...
        public SybaseHelper(string connstring)
        {            
        ...
        {
            using (conn = new SybaseConnection())
            {
        ...
        public bool ExecuteNonQueryTransaction(IList<string> sqls)
        {
            using (conn = new SybaseConnection())
            {
                ...
                SybaseTransaction st = conn.BeginTransaction();

                int sql_count = sqls.Count;

                //先用泛型动态获取SybaseCommand
                Cmds = new List<SybaseCommand>();
                for (int i = 0; i < sql_count; i++)
                {
                    Cmd = new SybaseCommand(sqls[i], conn);
                    Cmd.Transaction = st;
                    Cmds.Add(Cmd);
                }
        ...
        public IDataReader AspNetPagerProcedure(Wuqi.Webdiyer.AspNetPager pager, string sql)
        {
            ...
            parameters.Add(new SybaseParameter("@pageindex", pager.CurrentPageIndex));
            parameters.Add(new SybaseParameter("@pagesize", pager.PageSize));
            parameters.Add(new SybaseParameter("@docount", "0"));
        ...
        protected void PrepareCommand(CommandType cmd_type, string sql, IList<IDataParameter> parameters)
        {
            ...
            Cmd = new SybaseCommand(sql, conn);
            Cmd.CommandType = cmd_type;
            if (parameters != null)
            {
                foreach (IDataParameter sp in parameters)
                {
                    if (sp is SybaseParameter)
                    {
                        Cmd.Parameters.Add(sp);
                    }
                    else
                    {
                        throw new Exception("参数IDataParameter中含有非SybaseParameter的对象");
                    }
                }
            }
        }
        ...
        {
            return new SybaseConnection(this._connstring);
        }
