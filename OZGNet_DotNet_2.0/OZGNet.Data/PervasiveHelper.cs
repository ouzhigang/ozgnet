...
{
    /// <summary>
    /// Pervasive专用(依赖AspNetPager.dll),实现IDataHelper接口
    /// </summary>
    public class PervasiveHelper : IDataHelper
    {
        ...
        /// </summary>
        public PsqlCommand Cmd;
        /// <summary>
        /// 获取当前执行的ExecuteNonQueryTransaction()所有Command对象
        /// </summary>
        public IList<PsqlCommand> Cmds;
        private PsqlDataReader reader;
        private PsqlDataAdapter da;
        private DataSet dataset;

        /// <summary>
        /// 实例化PervasiveHelper
        /// </summary>
        public PervasiveHelper()
        { }
        ...
        public PervasiveHelper(string connstring)
        {            
        ...
        public bool ExecuteNonQueryTransaction(IList<string> sqls)
        {
            using (conn = new PsqlConnection())
            {
                ...
                PsqlTransaction st = conn.BeginTransaction();

                int sql_count = sqls.Count;

                //先用泛型动态获取PsqlCommand
                Cmds = new List<PsqlCommand>();
                for (int i = 0; i < sql_count; i++)
                {
                    Cmd = new PsqlCommand(sqls[i], conn);
                    Cmd.Transaction = st;
                    Cmds.Add(Cmd);
                }
        ...
        public IDataReader AspNetPagerProcedure(Wuqi.Webdiyer.AspNetPager pager, string sql)
        {
            ...
            parameters.Add(new PsqlParameter("@pageindex", pager.CurrentPageIndex));
            parameters.Add(new PsqlParameter("@pagesize", pager.PageSize));
            parameters.Add(new PsqlParameter("@docount", "0"));
        ...
        protected void PrepareCommand(CommandType cmd_type, string sql, IList<IDataParameter> parameters)
        {
            ...
            Cmd = new PsqlCommand(sql, conn);
            Cmd.CommandType = cmd_type;
            if (parameters != null)
            {
                foreach (IDataParameter sp in parameters)
                {
                    if (sp is PsqlParameter)
                    {
                        Cmd.Parameters.Add(sp);
                    }
                    else
                    {
                        throw new Exception("参数IDataParameter中含有非PsqlParameter的对象");
                    }
                }
            }
        }
        ...
        {
            return new PsqlConnection(this._connstring);
        }
