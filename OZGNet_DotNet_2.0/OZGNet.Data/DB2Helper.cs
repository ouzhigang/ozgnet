...
{
    /// <summary>
    /// DB2专用(依赖AspNetPager.dll),实现IDataHelper接口
    /// </summary>
    public class DB2Helper : IDataHelper
    {
        ...
        /// </summary>
        public DB2Command Cmd;
        /// <summary>
        /// 获取当前执行的ExecuteNonQueryTransaction()所有Command对象
        /// </summary>
        public IList<DB2Command> Cmds;
        private DB2DataReader reader;
        ...
        public DB2Helper(string connstring)
        {            
        ...
        public bool ExecuteNonQueryTransaction(IList<string> sqls)
        {
            using (conn = new DB2Connection())
            {
                ...
                DB2Transaction st = conn.BeginTransaction();

                int sql_count = sqls.Count;

                //先用泛型动态获取DB2Command
                Cmds = new List<DB2Command>();
                for (int i = 0; i < sql_count; i++)
                {
                    Cmd = new DB2Command(sqls[i], conn);
                    Cmd.Transaction = st;
                    Cmds.Add(Cmd);
                }
        ...
        public IDataReader AspNetPagerProcedure(Wuqi.Webdiyer.AspNetPager pager, string sql)
        {
            ...
            parameters.Add(new DB2Parameter("@pageindex", pager.CurrentPageIndex));
            parameters.Add(new DB2Parameter("@pagesize", pager.PageSize));
            parameters.Add(new DB2Parameter("@docount", "0"));
        ...
        protected void PrepareCommand(CommandType cmd_type, string sql, IList<IDataParameter> parameters)
        {
            ...
            if (parameters != null)
            {
                ...
                {
                    if (sp is DB2Parameter)
                    {
                        Cmd.Parameters.Add(sp);
                    }
                    else
                    {
                        throw new Exception("参数IDataParameter中含有非DB2Parameter的对象");
                    }
                }
            }
        }
        ...
        {
            return new DB2Connection(this._connstring);
        }
