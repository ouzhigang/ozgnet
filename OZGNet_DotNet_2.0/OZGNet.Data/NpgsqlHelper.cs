...
{
    /// <summary>
    /// PostgreSQL专用(依赖AspNetPager.dll),实现IDataHelper接口
    /// </summary>
    public class NpgsqlHelper:IDataHelper
    {
        ...
        /// </summary>
        public IList<NpgsqlCommand> Cmds;
        ...
        /// 实例化NpgsqlHelper
        /// </summary>
        /// <param name="PostgreSQL">PostgreSQL服务器</param>
        /// <param name="userName">用户名</param>
        /// <param name="passWord">密码</param>
        /// <param name="dbName">数据库</param>
        public NpgsqlHelper(string PostgreSQL, string userName, string passWord, string dbName)
        {
            connStringBuilder = new NpgsqlConnectionStringBuilder();
            connStringBuilder.Host = PostgreSQL;
            connStringBuilder.UserName = userName;
            connStringBuilder.Password = passWord;
            connStringBuilder.Database = dbName;
            connStringBuilder.Timeout = 30;
        ...
        public NpgsqlHelper(string PostgreSQL, string userName, string passWord, string dbName, int minPool, int maxPool)
        {
            if (maxPool <= minPool)
            {
            ...
            connStringBuilder.Database = dbName;
            connStringBuilder.Timeout = 30;
            connStringBuilder.MinPoolSize = minPool;
            connStringBuilder.MaxPoolSize = maxPool;
            ConnString = connStringBuilder.ConnectionString;
        } 
        #endregion
        ...
        {
            using (conn = new NpgsqlConnection())
            {
        ...
        public bool ExecuteNonQueryTransaction(IList<string> sqls)
        {
            using (conn = new NpgsqlConnection())
            {
                ...
                NpgsqlTransaction st = conn.BeginTransaction();

                int sql_count = sqls.Count;

                //先用泛型动态获取NpgsqlCommand
                Cmds = new List<NpgsqlCommand>();
                for (int i = 0; i < sql_count; i++)
                {
                    Cmd = new NpgsqlCommand(sqls[i], conn);
                    Cmd.Transaction = st;
                    Cmds.Add(Cmd);
                }
        ...
        protected void PrepareCommand(CommandType cmd_type, string sql, IList<IDataParameter> parameters)
        {
            ...
            if (parameters != null)
            {
                ...
                {
                    if (sp is NpgsqlParameter)
                    {
                        Cmd.Parameters.Add(sp);
                    }
                    else
                    {
                        throw new Exception("参数IDataParameter中含有非NpgsqlParameter的对象");
                    }
                }
            }
        }
        ...
        {
            return new NpgsqlConnection(this._connstring);
        }
