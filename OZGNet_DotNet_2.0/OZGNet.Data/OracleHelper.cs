...
{
    /// <summary>
    /// Oracle专用(依赖AspNetPager.dll),实现IDataHelper接口
    /// </summary>
    public class OracleHelper : IDataHelper
    {
        ...
        /// </summary>
        public IList<OracleCommand> Cmds;
        ...
        public bool ExecuteNonQueryTransaction(IList<string> sqls)
        {
            using (conn = new OracleConnection())
            {
                ...
                OracleTransaction st = conn.BeginTransaction();

                int sql_count = sqls.Count;

                //先用泛型动态获取OracleCommand
                Cmds = new List<OracleCommand>();
                for (int i = 0; i < sql_count; i++)
                {
                    Cmd = new OracleCommand(sqls[i], conn);
                    Cmd.Transaction = st;
                    Cmds.Add(Cmd);
                }
        ...
        public IDataReader AspNetPagerProcedure(Wuqi.Webdiyer.AspNetPager pager, string sql)
        {
            ...
            parameters.Add(new OracleParameter("@pageindex", pager.CurrentPageIndex));
            parameters.Add(new OracleParameter("@pagesize", pager.PageSize));
            parameters.Add(new OracleParameter("@docount", "0"));
        ...
        protected void PrepareCommand(CommandType cmd_type, string sql, IList<IDataParameter> parameters)
        {
            ...
            if (parameters != null)
            {
                foreach (IDataParameter sp in parameters)
                {
                    ...
                    {
                        throw new Exception("参数IDataParameter中含有非OracleParameter的对象");
                    }
                }
            }
        }
        ...
        {
            return new OracleConnection(this._connstring);
        }
