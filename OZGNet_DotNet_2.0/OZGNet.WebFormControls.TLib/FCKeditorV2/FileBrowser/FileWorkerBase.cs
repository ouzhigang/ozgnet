...
namespace OZGNet.WebFormControls.ThirdPart.FCKeditorV2.FileBrowser
{
	public abstract class FileWorkerBase : System.Web.UI.Page
	{
		...
		protected void FileUpload( string resourceType, string currentFolder, bool isQuickUpload )
		{
            ...
            if (!string.IsNullOrEmpty(ConfigurationManager.AppSettings["FCKeditor:AutoFileName"]))
            {
                if (Convert.ToBoolean(ConfigurationManager.AppSettings["FCKeditor:AutoFileName"]))
                {
                    //获取上传文件名（用当前日期来命名文件名）
                    sFileName = System.IO.Path.GetFileName(oFile.FileName);
                    sFileName = this.SanitizeFileName(sFileName);
                    sExtension = System.IO.Path.GetExtension(oFile.FileName);
                    sExtension = sExtension.TrimStart('.');
                    Random ro = new Random();
                    sFileName = DateTime.Now.ToString("yyyyMMddHHmmss") + ro.Next(99, 1000) + "." + sExtension;
                }
                else
                {
                    // 获取上传文件名（不修改文件名）
